...
k(Type(ref="double"),Dimension=1)),SampleTime(group="D0")=0)));
  model ModelWorkspace
    ...
    parameter RealAuto lateralPositionKD = 0.2 "Position loop derivative gain, including angle conversion" annotation(__MWORKS(BlockSystem(Type(inherit=InheritType.none,ref="double"))), HideResult=true);
    ...
    parameter RealAuto heightKI = 0.4 "Height loop integral gain" annotation(__MWORKS(BlockSystem(Type(inherit=InheritType.none,ref="double"))), HideResult=true);
    ...
    parameter RealAuto heightOutputMax = 18 "Height PID correction upper limit, not total thrust; total command is hoverThrust + heightPID.u" annotation(__MWORKS(BlockSystem(Type(inherit=InheritType.none,ref="double"))), HideResult=true);
